...
class BaseManager(object):
    ...
    def _get_data(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if response.status_code == 200:
                ...
                try:
                    raise ValidationError(_('%s') % XeroBadRequest(response))
                ...
                raise ValidationError(_('%s') % XeroUnauthorized(response))
                # raise XeroUnauthorized(response)

            elif response.status_code == 403:
                raise ValidationError(_('%s') % XeroForbidden(response))
                # raise XeroForbidden(response)

            elif response.status_code == 404:
                raise ValidationError(_('%s') % XeroNotFound(response))
                # raise XeroNotFound(response)

            elif response.status_code == 500:
                raise ValidationError(_('%s') % XeroInternalError(response))
                # raise XeroInternalError(response)

            elif response.status_code == 501:
                raise ValidationError(_('%s') % XeroNotImplemented(response))
                ...
                if payload:
                    raise ValidationError(_('%s') % XeroRateLimitExceeded(response))
                    # raise XeroRateLimitExceeded(response, payload)
                else:
                    raise ValidationError(_('%s') % XeroNotAvailable(response))
                    # raise XeroNotAvailable(response)
            else:
                raise ValidationError(_('%s') % XeroExceptionUnknown(response))
    ...
    def _put_attachment_data(
        self, id, filename, data, content_type, include_online=False, attachment_model=False
    ):
        """Upload an attachment to the Xero object."""
        # uri = "/".join([self.base_url, self.name, id, "Attachments", filename])
        uri = "/".join([self.base_url, attachment_model, id, "Attachments", filename])
        params = {"IncludeOnline": "true"} if include_online else {}
        headers = {"Content-Type": content_type, "Content-Length": str(len(data))}
        return uri, params, "put", data, headers, False

    def put_attachment(self, id, filename, file, content_type, include_online=False, attachment_model=False):
        """Upload an attachment to the Xero object (from file object)."""
        return self.put_attachment_data(
            id, filename, file.read(), content_type, include_online=include_online, attachment_model=attachment_model
        )
